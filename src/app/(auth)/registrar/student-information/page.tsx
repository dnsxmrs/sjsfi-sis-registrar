...
import { getStudentInformationTableData, getPersonalData, getHealthHistory} from '@/app/_actions/getStudentsList';
...
export default function StudentInformationPage() {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                const result = await getStudentInformationTableData();
                if (result.success) {
                    ...
                    if (result.students.length > 0) {
                        ...
                        handleStudentSelect(result.students[0].id);
                    }
                } else {
    ...
    useEffect(() => {
        if (activeTab === 'health' && selectedStudent) {
            const fetchHealthData = async () => {
                try {
                    const result = await getHealthHistory(selectedStudent.id);
                    if (result && result.success && result.students) {
                        setHealthData(result.students);
                    } else {
                        setHealthData(null);
    ...
    const filteredStudents = studentList.filter(student =>
        ...
        (student.applicationNumber && student.applicationNumber.toLowerCase().includes(searchTerm.toLowerCase()))
    );

    const handleStudentSelect = async (studentId: number) => {
        // Find and set the selected student from the list
        ...
        // Fetch personal data
        try {
            const result = await getPersonalData(studentId);
            if (result && result.success && result.students) {
                // Convert Date to string if needed
                const studentData = {
                    ...result.students,
                    birthdate: typeof result.students.birthdate === 'string'
                        ? result.students.birthdate
                        : new Date(result.students.birthdate).toISOString().split('T')[0]
                };
                setPersonalData(studentData);
            } else {
                console.error('Failed to fetch students:');
            }
        } catch (error) {
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-black">
                            {[
                                ...
                                <div key={status} className="inline-flex items-center">
                                    <div
                                        className={`w-4 h-4 border border-gray-400 rounded-full mr-2 ${father.statusOfParent === status ? "bg-gray-800" : ""
                                            }`}
                                    ></div>
                                    <span>{status}</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-black">
                                        {[
                                            ...
                                            <div key={status} className="inline-flex items-center">
                                                <div
                                                    className={`w-4 h-4 border border-gray-400 rounded-full mr-2 ${mother.statusOfParent === status ? 'bg-gray-800' : ''
                                                        }`}
                                                ></div>
                                                <span>{status}</span>
                                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-black">
                                        {[
                                            ...
                                            <div key={status} className="inline-flex items-center">
                                                <div
                                                    className={`w-4 h-4 border border-gray-400 rounded-full mr-2 ${guardian.statusOfParent === status ? 'bg-gray-800' : ''
                                                        }`}
                                                ></div>
                                                <span>{status}</span>
                                            </div>
                                    ...
                                    {filteredStudents.map((student) => (
                                        ...
                                        >
                                            ...
                                            <div className="text-xs text-gray-500 mt-1">
                                                {student.yearLevel} • {student.applicationNumber || student.id}
                                            </div>
                                        </div>
                                    ))}
                                </div>
