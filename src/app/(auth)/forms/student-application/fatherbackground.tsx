...
export default function FatherBackgroundPage({ onBack, onNext }: FatherBackgroundPageProps) {
  ...
  const calculateCompletion = (): number => {
    const filledCount = requiredFields.filter(field => {
      const value = fatherBackground[field as keyof typeof fatherBackground];
  ...
  const filteredCities = useMemo(() =>
    getFilteredCities(fatherBackground.stateProvince),
    [fatherBackground.stateProvince, getFilteredCities]
  );
  ...
  const handleInputChange = (field: keyof typeof fatherBackground, value: string) => {
    ...
    const updates: Partial<typeof fatherBackground> = { [field]: value };
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full bg-white rounded-lg shadow p-8 border border-gray-200 flex flex-col gap-6">
        ...
        {/* <div className={`p-4 rounded-lg border ${completionPercentage === 0 ? 'bg-gray-50 border-gray-200' :
            requiresValidation ? 'bg-yellow-50 border-yellow-300' : 'bg-blue-50 border-blue-200'
          }`}>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <p className="font-semibold text-sm">
                  {completionPercentage === 0 ? 'Optional Form - You may skip this page' :
                    requiresValidation ? 'Completion Required' : 'Optional - Continue filling or skip'}
                </p>
                <span className="text-sm font-medium">{Math.round(completionPercentage)}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                <div
                  className={`h-2 rounded-full transition-all duration-300 ${requiresValidation ? 'bg-yellow-500' : 'bg-blue-500'
                    }`}
              ...
              <p className="text-xs text-gray-600">
                {requiresValidation ?
                  `You've filled ${Math.round(completionPercentage)}% of the form. Please complete all required fields to proceed.` :
                  `Fill at least 50% to make this form required, or skip to the next page.`
                }
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-1">
            ...
            <select
              ...
              disabled={!fatherBackground.stateProvince}
            >
              <option value="">{fatherBackground.stateProvince ? 'Select City' : 'Select Province First'}</option>
              {cities.map((city) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              value={fatherBackground.companyCity}
